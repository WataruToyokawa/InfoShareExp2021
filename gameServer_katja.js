/*===============================================================
// Multi-argent two-armed bandit task with collective reward
// Author: Wataru Toyokawa
// Date: 08 May 2025
// Collaborating with Dr Katja Sangati
//
// Note:
...
const horizonList = [5, 10]//[30, 60]
, sessionNo = 0 // 0 = debug; 100~ = 30&31 July; 200~ = August; 300~ afternoon August;
, maxGroupSize = 10//8 // maximum size per group 
, minGroupSize = 1 // minimal group size below which the session becomes individual tasks
...
, changes = [17, 29, 45] // trials from which a new env setting starts
...
, firstRoomName = makeid(8) + '_debug_' + sessionNo
,	roomStatus = {}
, sessionNameSpace = {}
...
// Routings
const gameRouter = require('./routes/game_katja'); // loading game.ejs from which subjectID is transferred
...
roomStatus['decoyRoom'] = {
    ...
    share_or_not: createArray((horizonList[0]+horizonList[1]) * totalGameRound, maxGroupSize),
...
server.listen(port, function() {
	let logtxt = ' - Four-armed bandit task server listening on port ' + port + '; max_n_per_rooom = '+maxGroupSize;
	console.log(logtxt);
});
...
io.on('connection', function (client) {
	...
	if (typeof client.request._query.sessionName == 'undefined') {
		...
		let logtxt = '- sessionName was just assigned to '+ client.session;
		console.log(logtxt);
		...
		total_N_now++;
		sessionNameSpace[client.session] == 1;
		console.log(' - '+ client.session +' ('+client.subjectID+') in room '+client.room+' reconnected to the server');
	...
	client.on('core is ready', function(data) {
		if (client.started == 0) {
			client.started = 1
			// Time stamp
		    let logtext_coreReady = ' - Client: ' + client.session +'('+client.subjectID+') responds with an average latency = '+ data.latency + ' ms.';
	...
	client.on('choice made katja', function (data) {
		...
		if (data.num_choice > -1) {
			console.log(' - Client ' + client.session + ' (subNo = ' + client.subjectNumber + ') chose ' + data.num_choice + 'and got ' + data.individual_payoff + ' at t = ' + data.thisTrial + '.');
		} else {
			console.log(' - Client ' + client.session + ' (subNo = ' + client.subjectNumber + ') missed at t = ' + data.thisTrial + '.');
		}
    	...
    	if (typeof roomStatus[client.room] != 'undefined' & typeof client.subjectNumber != 'undefined') {
			...
			roomStatus[client.room]['saveDataThisRound'].push(
				...
				,	exp_condition: roomStatus[client.room]['exp_condition'] // groupPayoff or indivPayoff? 
				,	indivOrGroup: roomStatus[client.room]['indivOrGroup'] // 1 = group condition
				,	groupSize: roomStatus[client.room]['n']
				,	room: client.room // group ID
				,	confirmationID: client.session // confirmation code used for the payment
				,	subjectNumber: client.subjectNumber // ID number within the group (1 ~ n)
				,	subjectID: client.subjectID // prolificID or amazonID
				,	trial: roomStatus[client.room]['trial']
				,	gameRound: roomStatus[client.room]['gameRound']
				,	gameType: roomStatus[client.room]['taskOrder'][roomStatus[client.room]['gameRound']]
				,	chosenOptionLocation: data.chosenOptionLocation - 1 // the location of the chosen option (!= option id)
				,	chosenOptionID: data.num_choice // machine's id
				,	individual_payoff: data.individual_payoff // payoff generated by this individual action
				,	groupCumulativePayoff: roomStatus[client.room]['groupCumulativePayoff'][roomStatus[client.room]['gameRound']] // group's total payoff so far
				,	groupTotalPayoff: NaN
				,	dataType: 'choice' // 'choice' or 'group payoff'
				,	timeElapsed: timeElapsed // time since this round started
				,	latency: client.latency // latency of this client (not working properly?)
				...
				,	optionOrder_0: roomStatus[client.room]['optionOrder'][0] - 1
				,	optionOrder_1: roomStatus[client.room]['optionOrder'][1] - 1
				,	optionOrder_2: roomStatus[client.room]['optionOrder'][2] - 1
			...
			if (roomStatus[client.room]['doneNo'][ roomStatus[client.room]['pointer']-1 ] < roomStatus[client.room]['n']) {
				...
				if(typeof roomStatus[client.room]['round']!='undefined'&roomStatus[client.room]['round'] % 20 == 0) { 
					// save data to mongoDB
	...
	client.on('result feedback ended', function (data) {
		...
		if (data.thisTrial <= roomStatus[client.room]['horizon']) {
			...
			if (data.thisTrial < changes[0]) {
				this_env = 0
			} else if (data.thisTrial >= changes[0] & data.thisTrial < changes[1] ) {
				this_env = 1
			} else if (data.thisTrial >= changes[1] & data.thisTrial < changes[2] ) {
				this_env = 2
			} else if (data.thisTrial >= changes[2] ) {
				this_env = 3
			} 

			if (roomStatus[client.room]['gameType'] == 'static') this_env = 0;
			...
			roomStatus[client.room]['saveDataThisRound'].push(
				...
				,	groupSize: roomStatus[client.room]['n']
				,	room: client.room // group ID
				,	confirmationID: client.room // confirmation code used for the payment
				,	subjectNumber: client.room // ID number within the group (1 ~ n)
				,	subjectID: client.room// prolificID or amazonID
				,	trial: roomStatus[client.room]['trial']
				,	gameRound: roomStatus[client.room]['gameRound']
				,	this_env: this_env
				,	gameType: roomStatus[client.room]['taskOrder'][roomStatus[client.room]['gameRound']]
				,	chosenOptionLocation:  NaN // the location of the chosen option (!= option id)
				,	chosenOptionID: NaN  // machine's id
				,	individual_payoff: NaN // payoff generated by this individual action
				,	groupCumulativePayoff: roomStatus[client.room]['groupCumulativePayoff'][roomStatus[client.room]['gameRound']] // group's total payoff so far
				,	groupTotalPayoff: roomStatus[client.room]['groupTotalPayoff'][roomStatus[client.room]['pointer']-1]
				,	dataType: 'group payoff' // 'choice' or 'group payoff'
				,	timeElapsed: timeElapsed // time since this round started
				,	latency: client.latency // latency of this client (not working properly?)
				,	socialFreq: roomStatus[client.room]['socialFreq'][roomStatus[client.room]['pointer']-1]
				,	maxGroupSize: maxGroupSize
				,	currentEnv: roomStatus[client.room]['currentEnv']
				,	optionOrder_0: roomStatus[client.room]['optionOrder'][0]
				,	optionOrder_1: roomStatus[client.room]['optionOrder'][1]
				,	optionOrder_2: roomStatus[client.room]['optionOrder'][2]
				,	true_payoff_0: NaN 
				,	true_payoff_1: NaN
				,	true_payoff_2: NaN
				,	reactionTime: NaN
			...
			// updating trial, pointer, etc. 
			// then go to the next trial or end the session
			...
			// if this is executed,
			// there must be something wrong because it means
			// data.thisTrial > roomStatus[client.room]['horizon']
	...
	// 		if (data.share == 0) {
	// 			console.log(' - Client ' + client.subjectNumber + ' in ' + client.room + ' did NOT share the payoff info ' + data.payoff + ' at trial ' + data.thisTrial + '.');
	// 		} else {
	// 			console.log(' - Client ' + client.subjectNumber + ' in ' + client.room + ' shared the payoff info ' + data.payoff + ' at trial ' + data.thisTrial + '.');
	// 		}
			
	// 		if( typeof client.subjectNumber != 'undefined' ) {

	// 			if( roomStatus[client.room]['share_or_not'][roomStatus[client.room]['pointer']-1][client.subjectNumber-1] == null) {
	...
	// 				// console.log('share_or_not is: '+ JSON.stringify(roomStatus[client.room]['share_or_not'][client.subjectNumber-1]) );
	// 			}
	...
	// 			  	// if(typeof roomStatus[client.room]['trial']!='undefined'&roomStatus[client.room]['trial'] <= roomStatus[client.room]['horizon']) {
	...
	// 		}
	// 	} else {
	// 		// there is something wrong because data.thisTrial > roomStatus[client.room]['horizon']
	// 		io.to(client.session).emit('reconnected after the game started');
	// 	}
	// });
	...
	client.on("disconnect", function () {
		if(typeof client.room != 'undefined') {
			...
			*/
			...
			console.log(' - client disconnected: '+ client.session+' ('+client.subjectID+')'+' (room N: '+roomStatus[thisRoomName]['n']+', total N: '+total_N_now+')');
		}
	});
});
...
function proceedRound (room) {
	...
	roomStatus[room]['pointer']++; // pointer keep tracks round + (horizonList[0]+horizonList[1]) * gameRound
	if(roomStatus[room]['trial'] <= roomStatus[room]['horizon']) {
