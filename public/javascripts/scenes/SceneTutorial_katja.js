...
class SceneTutorial extends Phaser.Scene {
	...
	constructor (){
	    	super({ key: 'SceneTutorial_katja', active: false });
	}
	        ...
	        let gameTimer = this.time.addEvent({
	            ...
	            callback: function(){
	                ...
	                if(this.timeLeft < 0){
	                    ...
	                    tutorialPosition++;
						tutorialDiv.innerHTML = tutorialText[tutorialPosition];
						this.scene.start('SceneTutorialFeedback', { indivOrGroup: indivOrGroup, choice: -1, tutorialPosition: tutorialPosition });
						gameTimer.destroy();
	                }
	            },
	            callbackScope: this,
			...
			for (let i = 1; i < numOptions+1; i++) {
			    ...
			    this.options['box_active'+i].on('pointerdown', function (pointer) {
			    	...
			    	if (tutorialPosition < tutorialText.length) {
			    		choice_tutorial = i;
			    		//this.scene.stop('SceneTutorial_katja');
			    		this.scene.start('SceneTutorialFeedback', { indivOrGroup: indivOrGroup, choice: i, tutorialPosition: tutorialPosition });
			    		this.scene.stop('SceneTutorial_katja');
			    	} else {
